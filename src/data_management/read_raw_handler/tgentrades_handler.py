...
from src.enums.tgentrades_enum import TgentradesEnum
...
class TgentradesHandler(AbstractReadRawHandler):
    ...
    def _filter_by_ibex(self, df: pd.DataFrame, contracts_prefixes: t.List[str]) -> pd.DataFrame:
        ...
        df = df[df[TgentradesEnum.CONTRACT_CODE.value].str.startswith(tuple(contracts_prefixes), na=False)]

        # Select only futures and options with monthly maturity
        # We can identify them because their number of characters
        df = df[df[TgentradesEnum.CONTRACT_CODE.value].str.len().isin([
