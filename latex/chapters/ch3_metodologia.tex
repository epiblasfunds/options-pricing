...
\begin{itemize}
  ...
  \item \textbf{Lectura profesional del resultado:} el producto final no es solo una tabla de métricas, sino un modelo interpretable y servible.
\end{itemize}
...
En el cálculo de tipos se asume una aproximación operativa: no se dispone de un calendario TARGET2 explícito dentro del pipeline, por lo que el conteo de días hábiles se implementa con \texttt{pd.bdate\_range} (días laborables estándar). La composición se define mediante una regla explícita: se recorre cada día hábil del intervalo, se toma \(r_i\) (retrocediendo al último dato disponible si ese día no existe en la serie) y se multiplica
...
\textbf{El cuarto}, \texttt{Underlying Step}, garantiza el emparejamiento temporal sin lookahead y resuelve el precio de futuro subyacente mediante una unión temporal \emph{as-of}: para cada opción selecciona el último trade del futuro con instante de ejecución menor o igual al de la opción. Esta decisión evita lookahead dentro del propio ETL, como se ilustra en la Figura~\ref{fig:doc-data-underlying-1}.
...
\caption{\small Validación e interpretación del código de contrato.}
...
\caption{\small Construcción de features financieras.}
...
\caption{\small Motivación financiera de las features.}
...
\begin{itemize}
  ...
  \item Las \textbf{redes neuronales} sirven para comprobar si una arquitectura más flexible mejora realmente el problema o solo incrementa el coste y la complejidad de explicación.
...
\caption{\small Construcción de k-folds temporales.}
...
\begin{subfigure}[t]{0.60\textwidth}
    ...
    \includegraphics[
        ...
        keepaspectratio
    ]{../doc/mermaid-pdf/dashboard/sample-explainability/01.pdf}
...
\begin{subfigure}[t]{0.38\textwidth}
    ...
    \includegraphics[
        ...
        height=0.30\textheight,
        keepaspectratio
    ]{../doc/mermaid-pdf/dashboard/behaviour/volatility-surfaces/01.pdf}
    \caption{\small Construcción de superficies locales de volatilidad.}
    \label{fig:doc-dashboard-behaviour-volatility-surfaces-1}
\end{subfigure}

\caption{\small Diagramas de flujo de explicabilidad local y superficies de volatilidad.}
\label{fig:doc-dashboard-sample-and-surfaces}
\end{figure}

\vspace{-0.3cm}
\begin{figure}[h]
\centering
\includegraphics[width=0.9\textwidth]{../doc/mermaid-pdf/dashboard/diagnosis/01.pdf}%
...
La petición (\autoref{fig:api-request}) incluye modelo, tipo de opción, strike, subyacente, tiempo a vencimiento, tipo de interés y, opcionalmente, código de contrato y volatilidad real.

La respuesta (\autoref{fig:api-response}) de predicción incluye el modelo, la volatilidad estimada y la entrada normalizada. La respuesta de explicabilidad local añade waterfall SHAP, payload de contribuciones, vecinos históricos y distancias. API y dashboard comparten modelos y paquetes autocontenidos de dashboard, lo que reduce el riesgo de explicar una versión distinta de la usada para predecir. La operativa de workflows, automatización y despliegue reproducible que soporta esta separación se desarrolla con más detalle en el apéndice \ref{app:workflow-dev}.


\begin{figure}[!htpb]
\centering
\includegraphics[width=0.96\textwidth,height=0.56\textheight,keepaspectratio]{../imgs/api/request.png}%
\caption{\small Request contract of \texttt{POST /run\_model/sample\_explainability/}.}
\label{fig:api-request}
\end{figure}

\begin{figure}[!htpb]
\centering
\includegraphics[width=0.96\textwidth,height=0.56\textheight,keepaspectratio]{../imgs/api/response.png}%
\caption{\small Response contract of \texttt{POST /run\_model/sample\_explainability/}.}
...
La arquitectura del repositorio sigue una separación por funcionalidades: cada paquete tiene un cometido identificable y las salidas persistidas pasan de una etapa a otra sin mezclar entrenamiento, explicación y servicio. La Figura~\ref{fig:doc-repository-index-2} muestra la secuencia de ejecución, la cadena de salidas y las dependencias entre paquetes.
...
\texttt{src/enums} & Definición centralizada de columnas, tipos de contrato y tipos de opción. \\
