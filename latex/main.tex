...
La comparación final muestra que la solución estándar basada en Random Forest ofrece un comportamiento en test sólido. En el conjunto de test obtiene \(RMSE=0.088403\), \(MAE=0.054366\) y \(R^2=0.790941\). La lectura de resultados se organiza en tres planos: rendimiento predictivo, impacto del entrenamiento progressive ATM y capacidad de explicación del modelo de referencia mediante dashboard.
...
\begin{itemize}
  ...
  \item Las arquitecturas más complejas no aportan automáticamente mejor generalización, lo que refuerza la necesidad de validar en test y no por intuición tecnológica.
\end{itemize}
...
MAE test & 0.054366 \\
\(R^2\) test & 0.790941 \\
Argumento principal & Buen equilibrio entre error en test, estabilidad temporal y facilidad de uso en el paquete explicativo. \\
...
Más allá del RMSE, la comparación en train/validation muestra una generalización consistente: \(RMSE_{train}=0.071027\), \(RMSE_{val}=0.071080\) y \(R^2_{val}=0.653348\). XGBoost tiene el CE\(_2\) más bajo en train/validation, pero su test queda por detrás. La lectura práctica prioriza rendimiento en test, estabilidad razonable y menor degradación al pasar de validación a test.
...
\end{minipage}
\caption{\small SHAP global comparado en los dos modelos con mejor rendimiento en test: Random Forest y XGBoost.}
