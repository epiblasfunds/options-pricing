...
\end{minipage}
\caption{\small Comparativa de la pestaña \emph{Diagnosis} en los dos finalistas: Random Forest (modelo seleccionado) y XGBoost.}
...
\textbf{Implicación para la selección de modelo en producción.} Random Forest, bajo entrenamiento estándar (sin ATM), mantiene su posición como modelo seleccionado para la solución de producción. No solo logra las métricas más competitivas (RMSE 0.0884), sino que permanece robusto frente a variantes experimentales como ATM. Un modelo que no mejora con ATM es un indicador de que ya ha alcanzado un nivel de regularización apropiado: no es excesivamente sensible a ruidoen, y su capacidad de generalización es estable. Por el contrario, modelos que mejoran significativamente con ATM (Sequential NN, Quantum Inspired) sugieren que su arquitectura es intrínsecamente frágil y requeriría monitoreo cuidadoso en producción.

\textbf{Conclusión sobre Progressive ATM.} La técnica debe considerarse una variante experimental de refinamiento, útil únicamente para arquitecturas específicas que sufren de convergencia deficiente o sensibilidad a orden de datos. No constituye una mejora universal del pipeline estándar, y en algunos casos (XGBoost, Linear Regression) resulta innecesaria o contraproducente. Su valor académico está en la validación de que modelos de baja capacidad intrínseca pueden beneficiarse de estrategias de currículum; su aplicación práctica debe limitarse a escenarios donde se identifique explícitamente convergencia frágil durante ajuste de hiperparámetros. Para la solución final presentada, el entrenamiento estándar (sin Progressive ATM) es preferible: es más simple, es reproducible, y produce el modelo más robusto.
...
La Figura~\ref{fig:dash-tree-depth-rf-vs-xgb}, junto con la Tabla~\ref{tab:surrogate-rf} y la Tabla~\ref{tab:symbolic-fidelity-all-models}, permite una conclusión transversal: la facilidad para explicar un modelo mediante reglas o fórmulas compactas no coincide necesariamente con su capacidad predictiva final en test. El mejor modelo por error de mercado (Random Forest) es precisamente uno de los menos compresibles simbólicamente en esta ejecución, mientras que XGBoost y Sequential NN admiten surrogates simbólicos más fieles.
...
Con ello, queda cubierta la dimensión operativa solicitada por tutoría: valoración de ilíquidas, detección de anomalías, validación independiente, control de riesgos, soporte de cobertura y análisis de sensibilidad por moneyness/vencimiento, con trazabilidad explícita en resultados, discusión y anexo.
